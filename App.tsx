...
const App: React.FC = () => {
  ...
  const cacheImage = useCallback(async (url: string) => {
      ...
      try {
        ...
        if (response.ok) {
            ...
            await storeImageInCache(url, blob);
        }
      } catch (e) {
  ...
  const cacheAllTemplates = useCallback(async (templatesToCache: Template[], sessionId: number) => {
    ...
    while (templatesToProcess.length > 0 && sessionId === cachingSessionRef.current) {
        ...
        await Promise.all(templatesToProcess.map(async (template) => {
            ...
            try {
                await cacheImage(template.imageUrl);
  ...
  useEffect(() => {
    ...
    const paymentsListener = onValue(paymentsRef, (snapshot) => {
        ...
        fetchedPayments.forEach(p => {
            ...
            // GUARD: Check if we are NOT already in a session. If currentSessionKey exists, user is busy photographing.
            // Also ensure we are in a payment-related state to prevent random redirects during other flows.
            if (
                currentPaymentId === p.id && 
                p.status === 'verified' && 
                (appState === AppState.PAYMENT_SHOW || appState === AppState.PAYMENT_VERIFICATION) &&
                !isCreatingSessionRef.current &&
                !currentSessionKey // Important: Prevent starting a session if one is already active
            ) {
  ...
  const handleCreatePayment = useCallback(async (userName: string) => {
    ...
    const newPaymentRef = await push(ref(db, `data/${currentTenantId}/payments`), paymentData);
    if (newPaymentRef.key) {
  ...
  const renderContent = () => {
    ...
    switch (appState) {
      ...
      case AppState.PAYMENT_VERIFICATION:
          if (!selectedPriceList) { setAppState(AppState.WELCOME); return null; }
          return <PaymentVerificationScreen targetAmount={selectedPriceList.price} onVerified={handlePaymentVerified} onBack={handleBack} validateProofHash={validateProofHash} />;
